...
- [ ] **Failure-injection** tests: disk full mid-write, OOM, partial write of `sver.db`, follower lag spike. Not started.
...
- [ ] **Mixed-version cluster** test: an `R3` stream where some nodes run the feature build and some don't. Validates rolling upgrades. The feature is sufficiently invasive (apply-time header injection, file-backed checkpoint) that a mixed cluster could diverge in ways the leader-change + restart tests don't catch.
